        ...
        $html .= '
            <div class="gallery-entry">
                <div class="wmcz-caurosel-left">
                    <a href="' . $event->link . '">
                        ' .  wp_get_attachment_image( $event->photo_id, 'large' ) . '
                    </a>
                </div>
...
function wmcz_block_latest_news_render_callback( $attributes ) {
    ...
    $result .= '</div>';
    if ($q->found_posts > $maxNews && $maxNews != 0 && array_key_exists( 'tag', $attributes ) && $attributes['tag'] != null && $attributes['tag'] != '') {
        $tag = get_term_by( 'slug', $attributes['tag'], 'post_tag' );
        $result .= '<a href="' . get_tag_link( $tag->term_id ) . '">' . __( 'All news', 'wmcz-plugin' ) . '</a>';
    }
    $result .= '</div>';
    return $result;
}
